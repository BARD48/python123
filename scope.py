...
def collum(lines):
    ...
    return collum_set 
def regions(lines):
    #that function makes region set. And it starts by making top region set,middle region set, bottom region set. After that previous progress repeats left to rigth. Fınally previous progress
        ...
        while "0" in new_lines and not solution_found: #It checks 0 which it is there or isn't there
            for k in range(0,3):
                ...
                for j in range(0,3):
                    ...
                    for n in range(0+nine_increase,9+nine_increase):
                            ...
                            if solution_found: #It is for solving problem respectively. That logic like jumpn logic on assembly
                                break
                                    ...
                                    if len(solutions)==1: #It changes region set list,row list ,collum list to according to solution
                                        ...
                                        for m in range(0,9):
                                            r=[]
                                            for n in range(0,9): # It is for remove the comma
                                                r.append(new_lines[n+9*m])
                                                row_line=" ".join(r)
                                            row_lines_second.append(row_line)
                                        print(explaining_line)
